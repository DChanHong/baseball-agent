...
- 좌석 추천 요청은 find_kbo_game -> get_stadium_info -> get_weather_context -> search_baseball_knowledge -> score_seat_candidates 순서로 진행한다.
- 좌석 추천 최종 답변은 score_seat_candidates observation을 본 뒤에만 생성한다.
...
- 날씨 질문은 경기와 구장이 확정되면 get_stadium_info 후 get_weather_context를 호출한다.
- 예매/동선/좌석 설명은 정적 JSON을 직접 말하지 말고 Tool이 반환한 RAG 문서와 metadata를 근거로 답한다.
